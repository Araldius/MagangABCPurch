...
<div style="background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:22px 24px;">
    ...
    <div style="display:flex;gap:10px;align-items:center;">
        <div style="position:relative;flex:1;">
            <select id="pr-select" style="width:100%;padding:10px 36px 10px 14px;border:1px solid #d1d5db;border-radius:8px;font-size:13.5px;color:#374151;background:#fff;appearance:none;cursor:pointer;font-family:inherit;outline:none;">
                <option value="">--- Select PR number to view vendor offers ---</option>
                @foreach($rfqs ?? [] as $r)
                <option value="{{ route('vendors.select', $r) }}">{{ $r->purchaseRequest->document_number }} | {{ $r->purchaseRequest->title }}</option>
                @endforeach
            </select>
        ...
        <button onclick="goToPR()" style="display:inline-flex;align-items:center;gap:7px;padding:10px 20px;background:#1e3a5f;color:#fff;border:none;border-radius:8px;font-size:13.5px;font-weight:600;cursor:pointer;white-space:nowrap;font-family:inherit;">
...
<div style="margin-bottom:14px;display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:10px;">
    <div>
        ...
        <h1 style="font-size:20px;font-weight:700;color:#111827;margin:0 0 3px;">Vendor Selection: <span style="color:#1e3a5f;">{{ $pr->document_number }}</span></h1>
    ...
    <div style="display:flex;align-items:center;gap:10px;">
        <span style="display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:999px;background:#fef3c7;font-size:12.5px;font-weight:600;color:#92400e;">
            <span style="width:7px;height:7px;border-radius:50%;background:#f59e0b;"></span>Awaiting Selection
        </span>
        <a href="{{ route('pr.list') }}" style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:13px;font-weight:600;color:#374151;text-decoration:none;">← Back</a>
    </div>
</div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead>
                <tr style="background:#f9fafb;">
                    ...
                    <th style="padding:9px 16px;text-align:left;font-size:10.5px;font-weight:700;color:#9ca3af;">ITEM ID</th>
            ...
            <tbody>
                ...
                <tr id="item-row-{{ $item->id }}" style="border-bottom:1px solid #f9fafb;">
                    ...
                    <td style="padding:11px 16px;font-family:'Courier New',monospace;font-size:12px;font-weight:600;color:#3b5bdb;">{{ $item->item_id ?? $item->item_code }}</td>
                    ...
                    <td style="padding:11px 16px;">
                        <span id="item-status-{{ $item->id }}" style="display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:999px;background:#fff7ed;font-size:12px;font-weight:600;color:#c2410c;">
...
<div style="display:grid;grid-template-columns:repeat({{ min($vendorQuotations->count(), 3) }},1fr);gap:14px;margin-bottom:80px;">
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
        <div style="padding:13px 16px;border-bottom:1px solid #f3f4f6;background:#fafafa;">
            ...
            <div style="font-size:12px;color:#9ca3af;margin-top:2px;">{{ optional($vendor)->location ?? '—' }}</div>
        </div>

        <div style="padding:12px;display:flex;flex-direction:column;gap:10px;">
            @foreach($items as $item)
            @php
                $detail = $vendorQuotationRecord ? $vendorQuotationRecord->quotationDetails->where('purchase_request_item_id', $item->id)->first() : null;
            ...
            <div class="vendor-item-card"
                 ...
                 @if(!$isNotOffered) onclick="toggleItemCard(this)" onmouseover="if(this.dataset.selected!=='true')this.style.borderColor='#93c5fd'" onmouseout="if(this.dataset.selected!=='true')this.style.borderColor='#e5e7eb'" @endif>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;font-size:12px;">
                    <div><span style="color:#9ca3af;font-size:11px;">Qty Offer</span><br><span style="font-weight:600;color:#111827;">{{ $formattedQty }}{!! $qtyBadge !!}</span></div>
                    <div><span style="color:#9ca3af;font-size:11px;">Unit Price</span><br><span style="font-weight:600;color:#111827;">{{ $formattedPrice ?? '—' }}<span class="best-price-badge" data-item="{{ $item->id }}" style="display:none;"></span></span></div>
                    <div style="grid-column: span 2;"><span style="color:#9ca3af;font-size:11px;">Subtotal</span><br><span style="font-weight:600;color:#111827;">{{ $formattedSubtotal ?? '—' }}</span></div>
                </div>
                @else
                <div style="text-align:center;padding:8px;background:#fef2f2;border-radius:6px;font-size:12px;font-weight:600;color:#dc2626;">❌ NOT OFFERED</div>
                @endif
            </div>
            @endforeach
...
<div id="summary-section" style="display:none;margin-bottom:90px;">
    <div style="font-size:13.5px;font-weight:600;color:#374151;margin-bottom:12px;">Summary Selection for <span style="font-family:'Courier New',monospace;color:#1e3a5f;">{{ $pr->document_number }}</span></div>
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:14px;">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <thead>
                    <tr style="background:#f9fafb;">
                        <th style="padding:9px 16px;text-align:left;font-size:10.5px;font-weight:700;color:#9ca3af;">NO</th>
                        <th style="padding:9px 16px;text-align:left;font-size:10.5px;font-weight:700;color:#9ca3af;">ITEM NAME</th>
                        <th style="padding:9px 16px;text-align:right;font-size:10.5px;font-weight:700;color:#9ca3af;">QTY</th>
                        <th style="padding:9px 16px;text-align:right;font-size:10.5px;font-weight:700;color:#9ca3af;">UNIT PRICE (RP)</th>
                        <th style="padding:9px 16px;text-align:right;font-size:10.5px;font-weight:700;color:#9ca3af;">TOTAL (RP)</th>
                ...
                <tfoot>
                    <tr style="background:#f9fafb;">
                        <td colspan="4" style="padding:11px 16px;text-align:right;font-weight:700;border-top:1px solid #e5e7eb;">Grand Total:</td>
...
<div style="position:fixed;bottom:0;left:0;right:0;z-index:100;background:#fff;border-top:1px solid #e5e7eb;padding:11px 28px;display:flex;align-items:center;justify-content:space-between;gap:16px;box-shadow:0 -4px 20px rgba(0,0,0,.06);flex-wrap:wrap;">
    ...
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
        ...
        <button id="show-result-btn" onclick="showSelectionResult()" style="display:inline-flex;align-items:center;gap:7px;padding:9px 20px;background:#374151;color:#fff;border:none;border-radius:8px;font-size:13.5px;font-weight:700;cursor:pointer;opacity:.5;pointer-events:none;transition:.2s;">Show Selection Result</button>
    </div>
</div>
...
<form id="submit-form" action="{{ route('vendors.store.selection') }}" method="post" style="display:none;">@csrf</form>
...
function showSelectionResult() {
    ...
    for (const [itemId, sel] of Object.entries(selectedItems)) {
        ...
        tr.innerHTML = `
            ...
            <td style="padding:11px 16px;text-align:right;">${sel.subtotal ? 'Rp '+Number(sel.subtotal).toLocaleString('id-ID') : '—'}</td>
