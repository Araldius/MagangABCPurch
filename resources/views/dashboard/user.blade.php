        ...
        <table style="width:100%;border-collapse:collapse;font-size:12.5px">
            ...
            <tbody id="dash-h-tbody">
                ...
                @php
                    ...
                    $totalVal = $h->selectionItems->sum(fn($si)=>($si->final_price_per_item??0)*($si->final_quantity??0));
    ...
    return `<div style="display:flex;align-items:flex-start;gap:0;margin-bottom:20px">
        ${steps.map((s,i)=>{
            ...
            return `<div style="display:flex;flex-direction:column;align-items:center;flex:1;position:relative">
                ${i<3?`<div style="position:absolute;top:14px;left:50%;width:100%;height:2px;background:${done?'#22c55e':'#e5e7eb'};z-index:0"></div>`:''}
                <div style="width:28px;height:28px;border-radius:50%;background:${cb};color:${cc};font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;position:relative;z-index:1;flex-shrink:0">${ct}</div>
...
/* ══ TABLE ENGINE ══ */
function makeEngine(tbodyId,pagerId,pageSize){
    return { page:1, pageSize, sortCol:null, sortDir:'asc',
        rows(){ return Array.from(document.querySelectorAll('#'+tbodyId+' tr[data-status]')); }
    };
}
function runEngine(eng, filtered){
    eng.rows().forEach(r=>r.style.display='none');
    const pages=Math.max(1,Math.ceil(filtered.length/eng.pageSize));
    if(eng.page>pages)eng.page=1;
    const start=(eng.page-1)*eng.pageSize, end=Math.min(eng.page*eng.pageSize,filtered.length);
    const tbody=document.getElementById(eng.tbodyId||'');
    filtered.slice(start,end).forEach(r=>{r.style.display='';});
    renderPager(eng.pagerId,eng,pages,start,end,filtered.length);
}
